...
func TestValidateOriginalURL(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "too long",
			rawURL:    "https://example.com/" + strings.Repeat("a", MaxOriginalURLLength),
			wantError: true,
		},
		{
			name:      "missing host",
...
func TestValidateCode(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "letters digits hyphen underscore", code: "abc_123-Z"},
		{name: "empty", code: "", wantError: true},
		{name: "too long", code: strings.Repeat("a", CodeMaxLength+1), wantError: true},
		{name: "slash", code: "abc/def", wantError: true},
		{name: "unicode", code: "абв", wantError: true},
	}
