...
func (r *Repository) CountClicksByMonth(ctx context.Context, linkID uuid.UUID, filter analytics.ClickFilter) ([]analytics.TimeBucketCount, error) {
	...
	SELECT date_trunc('month', clicked_at) AS bucket, COUNT(*)
	FROM clicks
	`)
	...
	if err != nil {
		return nil, fmt.Errorf("query monthly click counts: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&count.Bucket, &count.Count); err != nil {
			return nil, fmt.Errorf("scan monthly click count: %w", err)
		}
		counts = append(counts, count)
	}

	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate monthly click counts: %w", err)
	}
